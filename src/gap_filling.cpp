...
int GapFilling::run(const Properties& options, const Arguments& arguments) {
	...
	if (options.find("f") != options.not_found()) {
        scaf_file_name = options.get< std::string >("f");
    }
    ...
    // write data
    {
        ...
        boost::filesystem::ofstream stream(scaf_file_name);
        if (!stream) {
            LOG4CXX_ERROR(logger, boost::format("Create %s error!!!") % scaf_file_name);
            return 2;
        }
        stream << gf;
    }
...
GapFilling::GapFilling() : Runner("c:s:d:K:O:C:I:l:f:h", boost::assign::map_list_of('O', "MAX_OVERLAP")) {
...
int GapFilling::printHelps() const {
    ...
    std::cout << "\t-I[=<file>]    <scaf_after_3rd>" << std::endl;
